...
как $\sup$ и $\inf$:
\begin{align*}
    \sup p &= \min u : \forall s \in p ~ u \ge s \\
    \inf p &= \max u : \forall s \in p ~ u \le s \\
    a + b  &= \sup ~ \{a, b\} \\
    a * b  &= \inf ~ \{a, b\}
\end{align*}
...
на множестве $\R^n$. Тогда заключения о общезначимости формулы
...
\begin{itemize}
    ...
    \item Любая представимая в ФА ф-я является рекурсивной
        \begin{multline*}
            f(x_1, \dotsc, x_n) = \\
            \plog(\mu \template{S\template{G_\phi, U_{n+1, 1}, \dotsc, U_{n+1, n},
            \plog(U_{n+1, n+1}, 1),
            \plog(U_{n+1, n+1}, 2)}}(x_1,\dotsc, x_n), 1)
        \end{multline*}

    $G_\phi$ тут принимает $n + 2$ аргумента: $x_1\ldots{}x_n, p, b$ и возвращает 0 если
    p -- доказательство $\phi(x_1, \dotsc, x_n, p)$, представляющего f.
\end{itemize}
...
\subsection*{Consis}
\label{sec-2-35}
Consis -- утверждение, формально доказывающее непротиворечивость ФА. То есть $\vdash Consis \Leftrightarrow \text{ФА непротиворечива}$
...
\begin{enumerate}
\item $\vdash a \Rightarrow \vdash \pi(\overline{\Godel a})$
\item $\vdash \pi \left(\overline{\Godel a}\right) \to \pi \left(\overline{\Godel{\pi (\overline{\Godel a})}}\right)$
\item $\vdash \pi \left(\overline{\Godel a}\right) \to
    \pi \left(\overline{\Godel{(a \to b)}}\right) \to \pi \left(\overline{\Godel{b}}\right)$
\end{enumerate}
\subsection*{Лемма о самоприменении}
\label{sec-2-37}
$a(x)$ -- формула, тогда $\exists b$ такой что
\begin{enumerate}
    \item $\vdash a\left(\overline{\Godel b}\right) \to b$
    \item $\vdash \beta \to a\left(\overline{\Godel b}\right)$
\end{enumerate}
